...
app.post("/Authenticate", async (req, res) => {
  ...
  connection.query(q, [], async function (err, user) {
    if (!user?.length)
      return res.status(400).send("Authentication failed, invalid credentials");

    connection.end();

    try {
      if (await bcrypt.compare(req?.body?.password, user[0].pw))
...
app.listen(3001, "localhost", () => console.log("Running on port 3001!"));
